...
function example(callback) {
    arpping.findMyInfo((err, info) => {
        ...
        arpping.arp([info.ip], (err, found) => {
            if (err) return console.log(err);
            if (found[0].type) arpping.search.byMacType(found[0].type, callback);
...
var tests = {
    ...
    discover: () => {
        test(arpping.discover, [], (time, err, hosts) => {
            console.log('\n--------------------------------');
    ...
    searchByIpAddress: (ips) => {
        test(arpping.search.byIpAddress, [ips], (time, err, found, missing) => {
            console.log('\n--------------------------------');
    ...
    searchByMacAddress: (macs) => {
        test(arpping.search.byMacAddress, [macs], (time, err, found, missing) => {
            console.log('\n--------------------------------');
    ...
    searchByMacType: (type) => {
        test(arpping.search.byMacType, [type], (time, err, found) => {
            console.log('\n--------------------------------');
            if (err) return console.log('Error during searchByMacType: ' + err);
            console.log('Found ' + found.length + ' hosts:\n' + JSON.stringify(found, null, 4));
            console.log('\nFinished search by mac type in ' + time + 's');
        });
    },
    ping: (ips) => {
...
if (input[3]) tests[input[2]](input[3].split(','));
else tests[input[2]]();